...
<ul class="pagination" style="margin: 0px">
    <li aria-label="Previous">
        <button type="submit" class="btn-link btn-personalizado" onclick="injectHiddenField('pageNumber', '${1}')">
            <a><span aria-hidden="true">«</span></a>
        </button>
    </li>
    <li aria-label="Previous">
        <button type="submit" class="btn-link btn-personalizado" ${(pageActual - 1) > 0 ? '' : 'disabled=true'}
    ...
    <g:each var="pagNumber"
        ...
        <g:if test="${pagNumber != pageActual}">
            <li>
                <button type="submit" class="btn-link btn-personalizado" onclick="injectHiddenField('pageNumber', '${pagNumber}')">
                    <a>${pagNumber}</a>
                </button>
            </li>
        </g:if>
    </g:each>
    <li class="active"><button  disabled="true" class="btn-link btn-personalizado">${pageActual}</button></li>
    ...
    <li aria-label="Previous">
        <button type="submit" class="btn-link btn-personalizado"
                onclick="injectHiddenField('pageNumber', '${pageActual + 1}')">
            <span aria-hidden="true">></span>
        </button>
    </li>
